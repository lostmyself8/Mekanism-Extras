...
public class ExtraGenSingleInputRecipeCache<INPUT,
        INGREDIENT extends InputIngredient<INPUT>,
        RECIPE extends MekanismRecipe & Predicate<INPUT>,
        CACHE extends IInputCache<INPUT, INGREDIENT, RECIPE>>
        extends ExtraGenAbstractInputRecipeCache<RECIPE> {

    private final Set<RECIPE> complexRecipes = new HashSet<>();
    private final Function<RECIPE, INGREDIENT> inputExtractor;
    private final CACHE cache;

    protected ExtraGenSingleInputRecipeCache(ExtraGenRecipeType<RECIPE, ?> recipeType, Function<RECIPE, INGREDIENT> inputExtractor, CACHE cache) {
        super(recipeType);

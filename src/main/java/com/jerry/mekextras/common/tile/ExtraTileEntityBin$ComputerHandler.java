...
public class ExtraTileEntityBin$ComputerHandler extends ComputerMethodFactory<ExtraTileEntityBin> {
    ...
    public ExtraTileEntityBin$ComputerHandler() {
        ...
        register(MethodData.builder("getCapacity", ExtraTileEntityBin$ComputerHandler::getCapacity_0).returnType(int.class).methodDescription("Get the maximum number of items the bin can contain."));
        register(MethodData.builder("isLocked", ExtraTileEntityBin$ComputerHandler::isLocked_0).returnType(boolean.class).methodDescription("If true, the Bin is locked to a particular item type."));

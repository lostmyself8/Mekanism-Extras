...
public class ExtraBinInsertRecipe extends ExtraBinRecipe {
            ...
            if (!stackInSlot.isEmpty()) {
                if (stackInSlot.getItem() instanceof ItemBlockExtraBin) {
                    if (!binStack.isEmpty() || stackInSlot.getCount() > 1) {
                        // If we already have a bin, or our first bin has a stack size greater than one then this is not
                        // a bin recipe
                        return false;
                    }
    ...
    public ItemStack assemble(CraftingInput inv, HolderLookup.Provider provider) {
        ...
        for (ItemStack stack : foundItems) {
            ...
            // TODO: This is part of what causes it to show a lower number than what potentially gets handled by the
            // container
            ...
            if (remainder.isEmpty()) {
                ...
                // We managed to insert some of it into the bin, just return our bin stack without marking it as being
                // from a recipe
    ...
    public NonNullList<ItemStack> getRemainingItems(CraftingInput inv) {
        ...
        for (ObjectIterator<Int2ObjectMap.Entry<ItemStack>> iterator = Int2ObjectMaps.fastIterator(foundSlots); iterator.hasNext();) {
            ...
            // Only try inserting a single item into the bin. We execute on a copy of the bin stack so that we can
            // mutate it and chain insertions
            // to validate if we can insert across multiple slots
            // TODO: Do we want to allow inserting more when we are acting as a SpecialQIORecipe? (Is that even the case
            // for this as it is the remainder)
    ...
    public static void onCrafting(ItemCraftedEvent event) {
        ...
        if (!result.isEmpty() && result.getItem() instanceof ItemBlockExtraBin) {
            ...
            if (fromRecipe != null && fromRecipe) {
                ...
                ExtraComponentBackedBinInventorySlot slot = convertToSlot(result);
                ...
                if (!storedStack.isEmpty()) {
                    ...
                    for (int i = 0, slots = craftingMatrix.getContainerSize(); i < slots; ++i) {
                        ...
                        if (stack.getCount() > 1 && ItemStack.isSameItemSameComponents(storedStack, stack)) {
                            // Try to insert any excess items in the slot (we lower it by one as the input slots have

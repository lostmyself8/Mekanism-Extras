 ...
 * @param blocks       all the block ids in the structure, each block is represented by a byte. This is sorted in Y, Z
 *                     and X
 *                     axis order, in other words, the index of the block with coordination (x,y,z) is
 *                     <code>(y * length + z) * width + x</code>.
 * @param addBlocks    extra bits that can be used to further define terrain; optional. Two nibbles are put into each
 *                     index
 *                     in this array. Unlike normal chunks, even indexes go on the high nibble and odd indexes go on the
 *                     low nibble.
 * @param data         block data additionally defining parts of the terrain. Only the lower 4 bits of each byte are
 *                     used.

...
public class ExtraBlockEnergyCube extends BlockTile.BlockTileModel<ExtraTileEntityEnergyCube, Machine<ExtraTileEntityEnergyCube>> {
    ...
    public ExtraBlockEnergyCube(Machine<ExtraTileEntityEnergyCube> type) {
        // Note: We require setting variable opacity so that the block state does not cache the ability of if blocks can
        // be placed on top of the energy cube
        // this may change based on what sides are enabled. Torches cannot be placed on the sides due to vanilla
        // checking the incorrect shape
    ...
    public boolean isPathfindable(@NotNull BlockState state, @NotNull BlockGetter world, @NotNull BlockPos pos, @NotNull PathComputationType pathType) {
        ...
        // allowed through this block it is not a full block. Otherwise, use the normal handling for if movement is
        // allowed
        return false;
    }

...
public class ExtraFactory<TILE extends TileEntityExtraFactory<?>> extends ExtraFactoryMachine<TILE> {
    ...
    public ExtraFactory(Supplier<TileEntityTypeRegistryObject<TILE>> tileEntityRegistrar, Supplier<ContainerTypeRegistryObject<? extends MekanismContainer>> containerRegistrar,
        ...
        if (tier.ordinal() < ExtraEnumUtils.EXTRA_FACTORY_TIERS.length - 1) {
            add(new ExtraAttributeUpgradeable(() -> ExtraBlocks.getAdvancedFactory(ExtraEnumUtils.EXTRA_FACTORY_TIERS[tier.ordinal() + 1], origMachine.getFactoryType())));
        }
    }
    ...
    public static class AdvancedFactoryBuilder<FACTORY extends ExtraFactory<TILE>, TILE extends TileEntityExtraFactory<?>, T extends ExtraMachineBuilder<FACTORY, TILE, T>>
        ...
        public static <TILE extends TileEntityExtraFactory<?>> AdvancedFactoryBuilder<ExtraFactory<TILE>, TILE, ?> createFactory(Supplier<?> tileEntityRegistrar, FactoryType type,
                                                                                                                                 ExtraFactoryTier tier) {
            AdvancedFactoryBuilder<ExtraFactory<TILE>, TILE, ?> builder = getAdvancedFactoryTILEAdvancedFactoryBuilder((Supplier<TileEntityTypeRegistryObject<TILE>>) tileEntityRegistrar, type, tier);
    ...
    private static <TILE extends TileEntityExtraFactory<?>> @NotNull AdvancedFactoryBuilder<ExtraFactory<TILE>, TILE, ?> getAdvancedFactoryTILEAdvancedFactoryBuilder(Supplier<TileEntityTypeRegistryObject<TILE>> tileEntityRegistrar, FactoryType type, ExtraFactoryTier tier) {
        AdvancedFactoryBuilder<ExtraFactory<TILE>, TILE, ?> builder = new AdvancedFactoryBuilder<>(new ExtraFactory<>(tileEntityRegistrar,
